...
class ESP32:
    ...
    async def resolve_device_ip(network_id):
        # We access the shared variable here
        import routes
    ...
    async def pair_device(db, user__id, ssid, pw):
        ...
        await ESP32.connect_to_esp_wifi()

        # 2. Prepare Config
        new_key = ESP32.generate_random_string(20)
        new_secret = ESP32.generate_random_string(32)
        ...
        try:
            ...
            info = await globals.mdns_bus.async_get_service_info("_http._tcp.local.", service_name)
    ...
    async def update_device(network_id, api_key, api_secret, new_value):
        ...
        ip = await ESP32.resolve_device_ip(network_id)
        if not ip: return
